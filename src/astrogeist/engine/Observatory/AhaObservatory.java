...
public final class AhaObservatory extends Observatory {
	...
	public AhaObservatory(IdNames idNames) {
		super("AhaObservatory", idNames);
...
//			.eraseTag(Tag.WHITE);
		...
		makeImaginingOpticalAxis(this);
		
		var sitesAxis = this.newAxis("sites1", "Sites");
	...
	private static Observatory makeImaginingOpticalAxis(Observatory o) {
		...
		var axis = o.newAxis("opt1", "Optical");
		...
		var c1 = axis.newConfiguration("LS60MT single stacked w b1200")
		  .addItem(zwoAsi174mm.name())
		  .addItem(b1200a.name())
		  .addItem(ls60mt.name())
		  .addItem(ls60Ha.name());
		
		var c2 = axis.newConfiguration("LS60MT single stacked w b1800", c1)
		  .replaceItem(b1200a.name(), b1800a.name());
		
		var c3 = axis.newConfiguration("LS60MT double stacked w b1200", c1)
		  .addItem(ls60Fha.name());
		
		var c4 = axis.newConfiguration("LS60MT double stacked w b1800", c2)
		  .addItem(ls60Fha.name());
		
		var c5 = axis.newConfiguration("LS60MT single stacked w Powermate 2.5x and b1200", c1)
			.addItemAfter(zwoAsi174mm.name(), pm25.name());
		
		var c6 = axis.newConfiguration("LS60MT double stacked w Powermate 2.5x and b1200", c5)
			.addItem(ls60Fha.name());
		
		var c7 = axis.newConfiguration("LS60MT single stacked w Powermate 2.5x and b1800", c2)
			.addItemAfter(zwoAsi174mm.name(), pm25.name());
		
		var c8 = axis.newConfiguration("LS60MT double stacked w Powermate 2.5x and b1800", c7)
			.addItem(ls60Fha.name());
		
		var c9 = axis.newConfiguration("LS60MT w Herschel")
			.addItem(zwoAsi174mm.name())
			.addItem(herschel.name())
			.addItem(ls60mt.name());
		
		var c10 = axis.newConfiguration("LS60MT w Powermate 2.5x and Herschel", c9)
			.addItemAfter(zwoAsi174mm.name(), pm25.name());
		
		var c11 = axis.newConfiguration("LS60MT w Baaded CaK and Herschel", c9)
			.addItemAfter(zwoAsi174mm.name(), caK.name())
			.eraseTag(Tag.WHITE);
	...
	private static Observatory makeSiteAxis(Observatory o) {
		return o;
	}
	
	public static void main(String[] args) {
		var obervatory = new AhaObservatory(new DefaultIdNames());
